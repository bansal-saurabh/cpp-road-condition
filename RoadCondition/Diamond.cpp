...
// Date: 1-01-2019
// Name: Saurabh Bansal
// This program defines constructors and functions for the Diamond class.
...
Diamond::Diamond(int pSize, const char* pShapeType, const char* pIdentifier, const char* pStatus, int pDate,
...
void Diamond::draw(const char* status, int size, int dx, int dy)
{
	int drawStart;
	string symbol;
	...
	for (int i = 1; i < size; i++)
	{
		...
		dataFile2.write(testString.c_str(), 2 * (size - i) - 1);

		testString.clear();

		drawStart = drawStart + 73;
	}
